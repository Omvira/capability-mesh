...
class HermesMeshMCPTools:
    ...
    def get_assignment_status(self, assignment_id: str) -> dict[str, Any]:
        ...
        raise HermesMeshClientError(f"unknown assignment_id: {assignment_id}")
...
def _missing_sdk_error() -> str:
    return (
        "The Python MCP SDK is required to run the stdio MCP server. "
        "Install it with `python -m pip install mcp` and retry."
    )
